...
class EventLogGenerator(object):
    def __init__(self, process_map=None, event_attributes=None, case_attributes=None):
        self.process_map = None
        self.likelihood_graph = None
        self.event_attributes = self._check_attributes(event_attributes)
        self.case_attributes = self._check_attributes(case_attributes)
    ...
    def build_likelihood_graph(self,
        ...
        for node in self.likelihood_graph:
            ...
            # TODO: prevent 0.00 due to rounding and normal distribution
            ...
            for successor, probability in zip(successors, probabilities):
                ...
                self.likelihood_graph.edges[node, successor]['probability'] = np.round(probability, 4)
    ...
    def generate(self,
        ...
        # Case attributes
        case_dependencies = {}
        if len(self.case_attributes) > 0:
            for variant in self.process_map.variants:
                key = '->'.join(variant.trace)
                case_dependencies[key] = []
                for case_attr in self.case_attributes:
                    case_dependencies[key].append(case_attr.create(10))
        ...
        for case_id, path in enumerate([random_walk(self.likelihood_graph) for _ in iter], start=1):
            ...
            for case_attr in case_attrs:
                case.attributes[case_attr.name] = case_attr.random_value()
            cases.append(case)
    ...
    def generate_without_process_map(self,
                 size,
                 anomalies=None,
                 anomaly_p=None,
                 anomaly_type_p=None,
    ...
    def persist_likelihood_graph(self, file_name):
        nx.write_gpickle(self.likelihood_graph, str(EVENTLOG_DIR / file_name))
