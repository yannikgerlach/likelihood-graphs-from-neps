...
class Anomaly(object):
    ...
    def generate_random_event(self):
        if self.activities is None:
            raise RuntimeError('activities has not been set.')
...
class AttributeAnomaly(Anomaly):
    ...
    def apply_to_case(self, case):
        ...
        for event_index in event_indices:
            ...
            available = [a for a in case[event_index].attributes.keys() if a in [a.name for a in self.attributes]]
            if len(available) == 0 or m > len(available):
                continue
            attribute_keys = sorted(np.random.choice(available, m, replace=False))
            attribute_indices = [[a.name for a in self.attributes].index(key) for key in attribute_keys]
    ...
    def pretty_label(label):
        ...
        changes = ', '.join([f'{a} at {i} was {o}' for a, i, o in zip(affected, index, original)])
...
def label_to_class(label, num_events, num_attributes):
    if label == 'normal':
