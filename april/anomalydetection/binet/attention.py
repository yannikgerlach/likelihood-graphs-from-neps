...
class Attention(tf.keras.layers.Layer):
    ...
    def dot_product(x, kernel):
        ...
        * (features,) = (batch_size, timesteps)
        ...
        """
        return tf.squeeze(tf.keras.backend.dot(x, tf.keras.backend.expand_dims(kernel)), axis=-1)
    ...
    def build(self, input_shape):
        ...
        # Setup time masks
        self.masks = tf.ones((input_shape[1] / self.concat_len, input_shape[1] / self.concat_len))
        self.masks = tf.linalg.band_part(self.masks, -1, 0)

        if self.concat_len > 1:
            self.masks = tf.tile(self.masks, [1, self.concat_len])

        super(Attention, self).build(input_shape)

    @staticmethod
    def masked_softmax(alpha, mask):
        """Masks alpha and then performs softmax, as Keras' softmax doesn't support masking."""
        alpha = tf.keras.backend.exp(alpha)
        if mask is not None:
            alpha *= tf.keras.backend.cast(mask, tf.keras.backend.floatx())

        partition = tf.keras.backend.cast(
            tf.keras.backend.sum(alpha, axis=1, keepdims=True) + tf.keras.backend.epsilon(), tf.keras.backend.floatx())
        return alpha / partition

    def call(self, x, mask=None):
        v_att = tf.keras.backend.dot(x, self.W)
        if self.bias:
            v_att += self.b
        v_att = tf.keras.backend.tanh(v_att)
        ...
        masks = tf.keras.backend.permute_dimensions(masks, [1, 0, 2])

        alpha = self.dot_product(v_att, self.u)  # > (batch, seq_len)

        alphas = tf.keras.backend.map_fn(lambda mask: tf.keras.backend.expand_dims(self.masked_softmax(alpha, mask)),
                                         masks)
        alphas = tf.keras.backend.permute_dimensions(alphas, [1, 0, 2, 3])

        x = tf.keras.backend.expand_dims(x, axis=1)
        attended_x = x * alphas

        alphas = tf.keras.backend.squeeze(alphas, axis=3)

        cs = tf.keras.backend.sum(attended_x, axis=2)

        if self.return_sequences:
            alpha = alphas
            c = cs
        else:
            alpha = tf.keras.backend.gather(alphas, -1, axis=1)
            c = tf.keras.backend.gather(cs, -1, axis=1)
