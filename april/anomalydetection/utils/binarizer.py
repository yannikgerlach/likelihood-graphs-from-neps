...
class Binarizer(object):
    ...
    def __init__(self, predictions, scores, mask, features, targets=None):
        ...
        if self._mask.shape != self.scores.shape:
            if len(self._mask) != len(self.scores.shape):
                self._mask = np.expand_dims(self._mask, axis=-1)
            self._mask = np.repeat(self._mask, self.scores.shape[-1], axis=-1)
    ...
    def correct_shape(self, tau, strategy):
        ...
        if strategy == Strategy.POSITION_ATTRIBUTE:
            tau = tau.reshape(*self.scores.shape[1:])
        return tau
    ...
    def classify(self, tau, features, predictions):
        ...
        if self.predictions is not None:
            ...
            # Top-1 predictions
            # p = np.argmax(self.predictions[0], axis=2) + 1

            # Top-n predictions according to threshold (tau)
            _p = self.get_scores(self.predictions[0])
        ...
        return self.apply_mask(classification)
