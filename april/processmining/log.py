...
class EventLog(object):
    ...
    @staticmethod
    def from_xes(file_path, classifier=0, remove_attributes=[], rename_attributes={}):
        """
        ...
        def parse_event(event):
            ...
            # remove the specified attributes from the event
            for attribute_name in remove_attributes:
                if attribute_name in attr.keys():
    ...
    def from_sql(server, database, resource, password, schema='pm'):
        ...
        cursor = conn.cursor()
        cursor.execute(f'SELECT CaseId, ActivityName, Resource, Timestamp FROM ' \
                       f'{schema}.EventLog ORDER BY CaseId, Timestamp, SortKey, ActivityName')
