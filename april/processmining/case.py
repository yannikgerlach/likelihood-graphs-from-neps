...
class Case(object):
    ...
    def attribute_names(self):
        return sorted(set().union(*(event.attributes.keys() for event in self.events)))
    ...
    def dataframe(self):
        return pd.DataFrame(
            [[self.id, e.name, *[e.attributes[key] for key in self.attribute_names]] for e in self.events],
            columns=['id', 'name', *self.attribute_names]
        )
