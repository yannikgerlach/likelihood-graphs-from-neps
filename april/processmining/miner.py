...
class HeuristicsMiner(Miner):
    ...
    def mine(self, features):
        ...
        if self.all_tasks_connected:
            start_symbol = features.max()
            end_symbol = features.max() - 1

            def check_node(graph, node):
                try:
                    if node != start_symbol:
                        nx.astar_path(graph, start_symbol, node)
                    if node != end_symbol:
                        nx.astar_path(graph, node, end_symbol)
                    return True
                except Exception:
                    return False
            ...
            def connected(mat):
                ...
                return all([check_node(graph, n) for n in graph.nodes])

            while not connected(self.adj_mat):
                candidates = np.max(dsr[~self.adj_mat])
                self.adj_mat[dsr == candidates] = True
